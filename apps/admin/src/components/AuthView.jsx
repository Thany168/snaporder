...
const AuthView = ({ onAuthSuccess }) => {
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      ...
      setLoginError(
        err.response?.data?.message || "Incorrect code. Use 111111.",
      );
    }
  };
      ...
      <div className="w-full max-w-md bg-white p-8 rounded-3xl border border-slate-100 shadow-xl text-slate-700 animate-in fade-in duration-200">
        ...
        {authStep === "password" ? (
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            <h2 className="text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">
              Sign In Context
            </h2>
            {loginError && (
            ...
            <div>
              ...
              <input
                type="email"
                required
                placeholder="admin@phumyerng.com"
            ...
            <button
              type="submit"
              className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold hover:bg-indigo-700 transition-all mt-2"
            >
              Next Step →
            </button>
          </form>
          ...
          >
            <h2 className="text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">
              Security Verification
            </h2>
            <p className="text-xs text-center text-slate-400 mb-4">
              A security code has been dispatched to {loginCredentials.email}
            </p>
            {loginError && (
              <div className="p-3 bg-rose-50 border border-rose-100 rounded-xl text-xs font-bold text-rose-600">
                {loginError}
            ...
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-widest flex justify-between">
                ...
                <span className="text-indigo-600 lowercase font-mono">
                  Test code: 111111
                </span>
              </label>
              <input
                ...
                required
                placeholder="000000"
                className="w-full mt-1 p-3 bg-slate-50 border border-slate-200 rounded-xl tracking-[1em] text-center font-bold text-lg outline-none focus:ring-2 focus:ring-indigo-500/20"
                value={otpCode}
            ...
            <button
              ...
              className="w-full text-center text-xs font-bold text-slate-400 hover:text-slate-600 mt-2 block transition-all"
            >
              ← Back to Password
            </button>
          </form>
